...
function memoize(func) {
  let cacher;
  let res = false;
  return function memoizeFun(...args) {
    if (!res) {
      cacher = func(...args);
      res = true;
    }
...
function logger(func, logFunc) {
  return function fun(...args) {
    const formArgs = JSON.stringify(args).slice(1, -1);
    const sig = `${func.name}(${formArgs})`;
    logFunc(`${sig} starts`);
    const res = func(...args);
    logFunc(`${sig} ends`);
    return res;
...
function partialUsingArguments(fn, ...args) {
  return function xz(...argsss) {
    return fn(...args, ...argsss);
  };
}
